...
/*!
    ...
    \param[in]  gpio_speed: gpio output max speed value
...
void gpio_init(uint32_t gpio_periph,uint32_t gpio_mode,uint32_t gpio_speed,uint32_t pin)
{
    ...
    temp_mode = (uint32_t)(gpio_mode & ((uint32_t)0x0FU));
    
    /* GPIO speed configuration */
    if(((uint32_t)0x00U) != ((uint32_t)gpio_mode & ((uint32_t)0x10U))){
        /* output mode max speed */
        if(GPIO_OSPEED_MAX == (uint32_t)gpio_speed){
            temp_mode |= (uint32_t)0x03U;
            /* set the corresponding SPD bit */
            GPIOx_SPD(gpio_periph) |= (uint32_t)pin ;
        }else{
            /* output mode max speed:10MHz,2MHz,50MHz */
            temp_mode |= (uint32_t)gpio_speed;
        }
    }
    ...
    for(i = 0U;i < 8U;i++){
        if((1U << i) & pin){
            ...
            /* set IPD or IPU */
            if(GPIO_MODE_IPD == gpio_mode){
                /* reset the corresponding OCTL bit */
                GPIO_BC(gpio_periph) = (uint32_t)pin;
            }else{
                /* set the corresponding OCTL bit */
                if(GPIO_MODE_IPU == gpio_mode){
...
/*!
    ...
    \param[in]  gpio_outputpin: GPIO_EVENT_PIN_x(x=0..15)
    \param[out] none
    \retval     none
*/
void gpio_event_output_config(uint8_t gpio_outputport,uint8_t gpio_outputpin)
{
    ...
    reg |= (uint32_t)((uint32_t)gpio_outputport << 0x04U);
    reg |= (uint32_t)gpio_outputpin;
...
{
    uint32_t source = 0U;
    source = ((uint32_t)0x0FU) << (AFIO_EXTI_SOURCE_FIELDS * (gpio_outputpin & (uint8_t)0x03U));

    /* select EXTI sources */
    if(GPIO_EVENT_PIN_4 > gpio_outputpin){
        /* select EXTI0/EXTI1/EXTI2/EXTI3 */
        AFIO_EXTISS0 &= ~source;
        AFIO_EXTISS0 |= (((uint32_t)gpio_outputport) << (AFIO_EXTI_SOURCE_FIELDS * (gpio_outputpin & (uint8_t)0x03U)));
    }else if(GPIO_EVENT_PIN_8 > gpio_outputpin){
        /* select EXTI4/EXTI5/EXTI6/EXTI7 */
        AFIO_EXTISS1 &= ~source;
        AFIO_EXTISS1 |= (((uint32_t)gpio_outputport) << (AFIO_EXTI_SOURCE_FIELDS * (gpio_outputpin & (uint8_t)0x03U)));
    }else if(GPIO_EVENT_PIN_12 > gpio_outputpin){
        /* select EXTI8/EXTI9/EXTI10/EXTI11 */
        AFIO_EXTISS2 &= ~source;
        AFIO_EXTISS2 |= (((uint32_t)gpio_outputport) << (AFIO_EXTI_SOURCE_FIELDS * (gpio_outputpin & (uint8_t)0x03U)));
    }else{
        /* select EXTI12/EXTI13/EXTI14/EXTI15 */
        AFIO_EXTISS3 &= ~source;
        AFIO_EXTISS3 |= (((uint32_t)gpio_outputport) << (AFIO_EXTI_SOURCE_FIELDS * (gpio_outputpin & (uint8_t)0x03U)));
    }
}
...
/*!
    ...
    \param[in]  gpio_remap: select the pin to remap
      ...
      \arg        GPIO_FSMC_NADV_REMAP: FSMC_NADV connect/disconnect
...
/*!
    ...
    \param[in]  gpio_compensation: specifies the I/O compensation cell mode
...
void gpio_compensation_config(uint32_t gpio_compensation)
{
    ...
    AFIO_CPSCTL = (reg | gpio_compensation);
}
