...
{ 
	...
	if(Is_Audio_Enable())
	{
		...
		else if(g_step==1)
		{
			...
			if(gmusic_value>=200)gmusic_value=200;
			dac_data_set(DAC0, DAC_ALIGN_8B_R, gmusic_value);
			speaker_num++;
			if(speaker_num>gsound_code_end)
			{				
				W25Q16_CS_OFF; 
				AUDIO_DISABLE;
				g_step=2;
...
void SPK_Report_Error(Error_t error_val)
{
	switch(error_val)
	{
		case ERROR_LEFT_WHEEL:    			
																		Speaker(SPK_CHECK_LEFT_WHEEL);
																    break;
		case ERROR_RIGHT_WHEEL:    			
																		Speaker(SPK_CHECK_RIGHT_WHEEL);
																    break;
		case ERROR_PICK_UP:    			
																		Speaker(SPK_PLE_START_FLOOR);
																    break;		
		case ERROR_CLIFF:    			
																		Speaker(SPK_CHECK_CLIFF);
																    break;		
		case ERROR_BUMPER:    			
																		Speaker(SPK_CHECK_BUMPER);
																    break;			
		case ERROR_SIDE_BRUSH:    			
																		Speaker(SPK_CHECK_SIDE_BRUSH);
																    break;	
		case ERROR_MAIN_BRUSH:    			
																		Speaker(SPK_CHECK_MAIN_BRUSH);
																    break;
		case ERROR_STUCK:    			
		...
		case ERROR_BATTERY:    			
																    ...
																    break;
		case ERROR_DUSTBIN:    			
																		Speaker(SPK_CHECK_DIRTY_TANK);																		
																    break;
		case ERROR_FAN_H:    			
																		Speaker(SPK_CHECK_FAN);
																    break;					
		default:break;	
	}	
}
