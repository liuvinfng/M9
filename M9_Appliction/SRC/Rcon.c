...
uint8_t Is_Turn_Remote(void)
{
  ...
  if(Rcon_GetRemoteCode()==Remote_Wall)return 1;
	return 0;
}
...
uint8_t Rcon_RemoteKey(uint32_t Key)
{
  if(Rcon_GetRemoteCode()==Key)
	{
    if( Key == Remote_Left || Key == Remote_Right || Key == Remote_Max ||Key == Remote_Forward ||Key == Remote_Home)
		{
...
{
  if(Rcon_GetStatus()&0X77FFF)return 1;
  return 0;
}

uint8_t Is_Meet_Station(void)
{
	return ((Rcon_GetStatus()&RconFront_Home_T)? 1 : 0);
}

uint8_t Is_Left_Meet_Station(void)
{
	return ((Rcon_GetStatus()&RconL_HomeT)? 1 : 0);
}

uint8_t Is_Right_Meet_Station(void)
{
	return ((Rcon_GetStatus()&RconR_HomeT)? 1 : 0);
}
